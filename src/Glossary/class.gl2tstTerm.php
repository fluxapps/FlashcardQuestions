...
class gl2tstTerm
{
    ...
    {
        global $ilDB;
        $set = $ilDB->query("SELECT value FROM adv_md_values_text WHERE field_id = 1 AND sub_type = 'term' AND obj_id = " . $ilDB->quote($this->getGlossary()->getId(), 'integer') . " AND sub_id = " . $ilDB->quote($this->term->getId(), 'integer'));

        return $ilDB->fetchObject($set)->value;
    }
    ...
    {
        global $ilDB;
        $set = $ilDB->query("SELECT value FROM adv_md_values_text WHERE field_id = 5 AND sub_type = 'term' AND obj_id = " . $ilDB->quote($this->getGlossary()->getId(), 'integer') . " AND sub_id = " . $ilDB->quote($this->term->getId(), 'integer'));
